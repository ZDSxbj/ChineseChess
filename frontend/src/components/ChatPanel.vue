...
<template>
  <div :class="['w-full flex flex-col bg-white rounded-lg shadow-sm', isMinimized ? 'h-12' : 'h-chat']">
    ...
    <div v-show="!isMinimized" ref="chatScrollRef" class="flex-1 p-4 overflow-y-auto chat-scroll">
      <div v-for="(msg, index) in messages" :key="index" class="mb-4">
        <div :class="['message-row', msg.sender === '我' ? 'self' : 'other']">
          <div :class="['bubble', msg.sender === '我' ? 'bubble--self' : 'bubble--other']">
    ...
    <div v-show="!isMinimized" class="px-3 py-3 border-t bg-gray-50">
      <div class="flex items-center space-x-3">
        <input
          v-model="inputMessage"
          type="text"
          :maxlength="maxLen"
          class="flex-1 px-3 py-2 border rounded text-sm outline-none focus:ring-2 focus:ring-blue-300"
          placeholder="请输入信息..."
...
.bubble { max-width: 70%; padding: 10px 12px; border-radius: 12px; box-shadow: 0 1px 0 rgba(0,0,0,0.02); }
.bubble--self { background: #007bff; color: white; border-bottom-right-radius: 4px; }
...
.bubble-time { font-size: 11px; color: rgba(0,0,0,0.45); margin-top: 6px; }

.chat-scroll::-webkit-scrollbar { width: 6px; }
.chat-scroll::-webkit-scrollbar-track { background: #f7fafc; }
.chat-scroll::-webkit-scrollbar-thumb { background: #cbd5e0; border-radius: 3px; }
